<template>
  <div>
    ...
    <div class="row">
      ...
      >
        <h5 class="title" style="margin: 4px 0px">Categoria de Receitas</h5>
        <div class="col-12 sub-title" style="text-align: start">
          Gerencie suas Categorias de Receitas
        </div>
      </div>
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <template v-if="col.name == 'actions'">
                  ...
                  <KButtonAction
                    ...
                    @click="showActiveInactiveRevenueCategory(props.row)"
                  />
                </template>
...
const items = ref<Array<IRevenueCategory>>([]);

const revenueCategory = ref<IRevenueCategory>({} as IRevenueCategory);

const dataBreadCrumbs = ref([
  {
    label: "Lista de Categorias de Receitas",
...
function showActiveInactiveRevenueCategory(item: IRevenueCategory) {
  revenueCategory.value = item;

  dialogMessageTitle.value = item.isActive
    ? "Desativar Categoria de Receita"
    : "Ativar Categoria de Receita";
  dialogMessageText.value = item.isActive
    ? "Deseja desativar a Categoria de Receita selecionada?"
    : "Deseja ativar a Categoria de Receita selecionada?";
...
function callActiveInactiveYes() {
  //console.log('teste')
  onActiveInactiveRevenueCategory(revenueCategory.value);
...
function callActiveInactiveNo() {
  dialogQuestion.value = false;
  revenueCategory.value = {} as IRevenueCategory;
}

async function showItemsRevenueCategory() {
  dialogLoading.value = true;
  let result: IResult;
  result = await Api.getResult<IResult>("/RevenueCategory/" + userLogged.id);
...
async function onActiveInactiveRevenueCategory(
  revenueCategory: IRevenueCategory
) {
  dialogLoading.value = true;
  revenueCategory.dateUpdate = new Date();
...
function showNewLevel() {
  router.push("/formRevenueCategory");
}

function onEdit(item: any) {
  //console.log(item);
  router.push("/editRevenueCategory/" + item.id);
}
