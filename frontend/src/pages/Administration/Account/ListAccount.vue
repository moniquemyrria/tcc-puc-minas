<template>
  <div>
    ...
    <div class="row">
      ...
      >
        <h5 class="title" style="margin: 4px 0px">Contas</h5>
        <div class="col-12 sub-title" style="text-align: start">
          Gerencie suas Contas pessoais ou bancárias
        </div>
      </div>
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <template v-if="col.name == 'actions'">
                  ...
                  <KButtonAction
                    ...
                    @click="showActiveInactiveAccount(props.row)"
                  />
                </template>
                ...
                <template v-else-if="col.name == 'accountCategory'">
...
const items = ref<Array<IAccount>>([]);
...
function showActiveInactiveAccount(item: IAccount) {
  account.value = item;
  ...
  dialogMessageText.value = item.isActive
    ? "Deseja desativar a Conta selecionada?"
    : "Deseja ativar a Conta selecionada?";
...
function callActiveInactiveYes() {
  //console.log('teste')
  onActiveInactiveAccount(account.value);
...
async function showItemsAccount() {
  ...
  let result: IResult;
  result = await Api.getResult<IResult>("/Account/" + userLogged.id);
...
async function onActiveInactiveAccount(
  account: IAccount
) {
  dialogLoading.value = true;
  account.dateUpdate = new Date();
...
function showNewItem() {
  router.push("/formAccount");
}

function onEdit(item: any) {
  //console.log(item);
  router.push("/editAccount/" + item.id);
}
