<template>
  <div>
    ...
    <div class="row">
      ...
      >
        <h5 class="title" style="margin: 4px 0px">Categoria de Contas</h5>
        <div class="col-12 sub-title" style="text-align: start">
          Gerencie suas Categorias de Contas
        </div>
      </div>
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <template v-if="col.name == 'actions'">
                  ...
                  <KButtonAction
                    ...
                    @click="showActiveInactiveAccountCategory(props.row)"
                  />
                </template>
...
const items = ref<Array<IAccountCategory>>([]);

const accountCategory = ref<IAccountCategory>({} as IAccountCategory);

const dataBreadCrumbs = ref([
  {
    label: "Lista de Categorias de Contas",
...
function showActiveInactiveAccountCategory(item: IAccountCategory) {
  accountCategory.value = item;

  dialogMessageTitle.value = item.isActive
    ? "Desativar Categoria de Conta"
    : "Ativar Categoria de Conta";
  dialogMessageText.value = item.isActive
    ? "Deseja desativar a Categoria de Conta selecionada?"
    : "Deseja ativar a Categoria de Conta selecionada?";
...
function callActiveInactiveYes() {
  //console.log('teste')
  onActiveInactiveAccountCategory(accountCategory.value);
...
function callActiveInactiveNo() {
  dialogQuestion.value = false;
  accountCategory.value = {} as IAccountCategory;
}

async function showItemsAccountCategory() {
  dialogLoading.value = true;
  let result: IResult;
  result = await Api.getResult<IResult>("/AccountCategory/" + userLogged.id);
...
async function onActiveInactiveAccountCategory(
  accountCategory: IAccountCategory
) {
  dialogLoading.value = true;
  accountCategory.dateUpdate = new Date();
...
function showNewItem() {
  router.push("/formAccountCategory");
}

function onEdit(item: any) {
  //console.log(item);
  router.push("/editAccountCategory/" + item.id);
}
