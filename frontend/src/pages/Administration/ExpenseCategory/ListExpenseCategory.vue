<template>
  <div>
    ...
    <div class="row">
      ...
      >
        <h5 class="title" style="margin: 4px 0px">Categoria de Despesas</h5>
              ...
              <q-td v-for="col in props.cols" :key="col.name" :props="props">
                <template v-if="col.name == 'actions'">
                  ...
                  <KButtonAction
                    ...
                    @click="showActiveInactiveExpenseCategory(props.row)"
                  />
                </template>
...
const items = ref<Array<IExpenseCategory>>([]);

const expenseCategory = ref<IExpenseCategory>({} as IExpenseCategory);

const dataBreadCrumbs = ref([
  {
    label: "Lista de Categorias de Despesas",
    router: "/listExpenseCategory",
  },
  {
    label: "Nova Categoria de Despesa",
    router: "/formExpenseCategory",
  },
]);
...
function showActiveInactiveExpenseCategory(item: IExpenseCategory) {
  expenseCategory.value = item;

  dialogMessageTitle.value = item.isActive
    ? "Desativar Categoria de Despesa"
    : "Ativar Categoria de Despesa";
  dialogMessageText.value = item.isActive
    ? "Deseja desativar a Categoria de Despesa selecionada?"
    : "Deseja ativar a Categoria de Despesa selecionada?";
...
function callActiveInactiveYes() {
  //console.log('teste')
  onActiveInactiveExpenseCategory(expenseCategory.value);
...
function callActiveInactiveNo() {
  dialogQuestion.value = false;
  expenseCategory.value = {} as IExpenseCategory;
}

async function showItemsExpenseCategory() {
  dialogLoading.value = true;
  let result: IResult;
  result = await Api.getResult<IResult>("/ExpenseCategory/" + userLogged.id);
...
async function onActiveInactiveExpenseCategory(
  expenseCategory: IExpenseCategory
) {
  dialogLoading.value = true;
  expenseCategory.dateUpdate = new Date();
...
function showNewLevel() {
  router.push("/formExpenseCategory");
}

function onEdit(item: any) {
  //console.log(item);
  router.push("/editExpenseCategory/" + item.id);
}
