    ...
    <div class="row">
      <div class="col-12" style="text-align: start">
        <h5 style="margin: 4px 0px 20px" class="title">
          {{ idExpenseCategory == "" ? "Nova Categoria" : "Editar Categoria" }}
        </h5>
        <div class="col-12 sub-title" style="text-align: start">
          {{
            idExpenseCategory == ""
              ? "Preencha os campos para criar uma nova Categoria de Despesa"
              : "Editar os campos da Categoria de Despesa"
          }}
        </div>
      </div>
    </div>
        ...
        <q-form class="q-gutter-md">
          <div class="row">
            <div class="col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2 col-xxl-2">
              <k-input
                v-model="expenseCategory.initials"
                filled
                label="Iniciais *"
              />
            </div>
...
const expenseCategory = ref(new IExpenseCategory());
const idExpenseCategory = ref("");
...
const dataBreadCrumbs = ref([
  {
    label: "Lista Categorias de Despesas",
...
function showList() {
  router.push("/listExpenseCategory");
}
...
function callItemsBreadCrumbs() {
  dataBreadCrumbs.value.push(
    route.params.id
      ? {
          label: "Editar Categoria de Despesa",
          router: "/",
        }
      : {
...
function validateFields() {
  if (!expenseCategory.value.description) {
...
async function onSave() {
  if (!validateFields()) {
    ...
    expenseCategory.value.idUser = userLogged.id;

    let result = await Api.postResult<IResult>(
      "/ExpenseCategory/" + userLogged.id,
...
async function onEdit() {
  if (!validateFields()) {
    dialogLoading.value = true;
    expenseCategory.value.dateUpdate = new Date();
...
async function showExpenseCategory() {
  if (route.params.id) {
    idExpenseCategory.value = route.params.id.toString();

    dialogLoading.value = true;
    let result: IResult = {} as IResult;
    result = await Api.getIdResult<IResult>(
      "/ExpenseCategory/Edit",
      idExpenseCategory.value.toString()
    );

    console.log(result.data);

    if (result.data.sucesso) {
      expenseCategory.value = result.data.tRetorno;
    }
